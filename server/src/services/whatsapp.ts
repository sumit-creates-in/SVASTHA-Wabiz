...
async function send(num: NumberLike, payload: Record<string, unknown>): Promise<SendResult> {
  try {
    const { data } = await api(num.tokenOverride).post(`/${num.phoneNumberId}/messages`, {
    ...
    const msg = e ? `${e.message}${e.error_data?.details ? ` — ${e.error_data.details}` : ""}` : err.message;
    console.error("[whatsapp] send error:", msg);
    return { error: msg || "send failed" };
  }
}

export function sendText(num: NumberLike, to: string, body: string): Promise<SendResult> {
...
export function sendTemplate(
  ...
  return send(num, { to, type: "template", template: { name, language: { code: language }, components } });
}

export async function markRead(num: NumberLike, waMessageId: string): Promise<void> {
  try {
...
export async function getMediaUrl(num: NumberLike, mediaId: string): Promise<string | undefined> {
  try {
...
export async function getAppId(token?: string): Promise<string | null> {
  ...
  try {
    const { data } = await api(t).get("/debug_token", { params: { input_token: t } });
...
export async function getSubscribedApps(businessAccountId: string, token?: string): Promise<SubscribedApp[]> {
  const { data } = await api(token).get(`/${businessAccountId}/subscribed_apps`);
...
export async function subscribeApp(businessAccountId: string, token?: string): Promise<boolean> {
  const { data } = await api(token).post(`/${businessAccountId}/subscribed_apps`);
  return !!data?.success;
}

/** Unsubscribe the token's own app from this WABA. */
export async function unsubscribeApp(businessAccountId: string, token?: string): Promise<boolean> {
  const { data } = await api(token).delete(`/${businessAccountId}/subscribed_apps`);
  return !!data?.success;
}

/** Fetch approved message templates for a WABA. */
export async function fetchTemplates(businessAccountId: string, token?: string): Promise<any[]> {
  const { data } = await api(token).get(`/${businessAccountId}/message_templates`, {
    params: { limit: 200 }
  });
  return data?.data || [];
}
...
export async function createTemplate(
  ...
  const { data } = await api(token).post(`/${businessAccountId}/message_templates`, {
    name: t.name,
    language: t.language,
    category: t.category,
...
export async function fetchPhoneNumbers(businessAccountId: string, token?: string): Promise<any[]> {
...
export async function fetchPhoneNumberHealth(phoneNumberId: string, token?: string): Promise<any> {
...
export async function syncNumberHealth(num: IWabaNumber): Promise<IWabaNumber> {
  ...
  try {
    const d = await fetchPhoneNumberHealth(num.phoneNumberId, num.tokenOverride);
    ...
    num.status = d.status || (d.code_verification_status === "VERIFIED" ? "CONNECTED" : "PENDING");
    ...
    if (num.messagingLimit === "UNKNOWN") {
      try {
        const list = await fetchPhoneNumbers(num.businessAccountId, num.tokenOverride);
        const match = list.find((p: any) => String(p.id) === num.phoneNumberId);
        if (match?.messaging_limit_tier) num.messagingLimit = match.messaging_limit_tier;
...
async function recordQuality(
  ...
  const changed = previousQuality !== num.qualityRating || previousLimit !== num.messagingLimit;
  ...
  if (!changed || previousQuality === "UNKNOWN" || num.qualityRating === "UNKNOWN") return;

  const degraded = (rank[num.qualityRating] ?? 0) < (rank[previousQuality] ?? 0);
  if (!degraded) {
  ...
  if (settings.autoPauseMarketingOnDegrade && critical) {
    ...
    console.warn(`[quality] ${num.displayPhoneNumber} went RED — running broadcasts cancelled`);
  }
}
