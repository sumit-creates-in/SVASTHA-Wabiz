 ...
 *  If the full path string is a direct key on obj (e.g. "Mobile No."), that
 *  exact key takes priority over dot-path traversal so that field names
 *  containing a literal dot are never mis-split. */
function readPath(obj: any, path: string): unknown {
  if (!path) return undefined;
  // Direct key lookup first — handles field names that contain literal dots
  // (e.g. "Mobile No."). Only falls through to dot-path split when the key
  // does not exist verbatim on the top-level object.
...
export async function fireWorkflow(
  ...
  const finish = async (
    ...
    await Workflow.updateOne(
      { _id: workflow._id },
      {
        $inc: { "stats.processed": 1, [incKey]: 1 },
        $set: { lastFiredAt: new Date(), lastError: reason },
      },
    );
  ...
  if (!waId || waId.length < 8)
    return finish("failed", `No valid phone at field "${workflow.phoneField}"`);
  ...
  if (workflow.dedupe !== "none") {
    ...
    if (prior)
      return finish("skipped", "Already sent to this contact (dedupe)");
  }
  ...
  const headerParams = workflow.headerParams.map((p) =>
  ...
  const conversation = await Conversation.findOneAndUpdate(
    { contact: contact._id, number: number._id },
    {
      ...
      $set: {
        ...
        lastMessagePreview: `[template] ${workflow.templateName}`,
      },
  ...
  await Message.create({
    ...
    author: "workflow",
    type: "template",
    text: renderTemplatePreview(
      tpl?.bodyText || workflow.templateName,
      bodyParams,
    ),
