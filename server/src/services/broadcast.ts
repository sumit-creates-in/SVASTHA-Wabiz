import { Broadcast, BroadcastRecipient, Contact, Template, WabaNumber, getSettings } from "../models";
...
export async function runBroadcast(broadcastId: string): Promise<void> {
  ...
  const category = (tpl?.category || "MARKETING").toUpperCase() === "MARKETING" ? "marketing" : "utility";
  ...
  for (const c of contacts) {
    ...
    const gate = canSendTemplate(c, number, settings, category as "marketing" | "utility");
    if (!gate.allowed) {
      ...
      await Broadcast.updateOne({ _id: b._id }, { $inc: { "stats.skipped": 1 } });
      continue;
    }

    const params = b.bodyParams.map((p) => p.replace(/\{\{name\}\}/gi, c.name || "there"));
    const result = await wa.sendTemplate(number, c.waId, b.templateName, b.templateLanguage, params);
    await BroadcastRecipient.create({
      broadcast: b._id,
      contact: c._id,
